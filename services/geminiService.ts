...
- **핵심 질문:** 이 글은 독자에게 **생각할 거리**를 던습니까, 아니면 **생각할 필요 없이** 특정 감정(분노, 불안, 혐오, 탐욕 등)을 느끼게 만듭니까?
...
- **종합 분석 리포트(comprehensiveAnalysis):** 분석 결과를 바탕으로 아래 형식에 맞춰 리포트를 작성해주세요. 만약 분석된 숨은 의도 유형이 없다면, 해당 텍스트가 **논리적 근거를 기반으로 한 비판적 의견이나 주장일 가능성이 높다는 점**을 명확히 밝혀주세요.
    - **매우 중요**: 리포트의 각 항목은 **분류된 개조식(bullet points)**으로 작성해주세요. **각 항목은 반드시 줄바꿈(newline)으로 구분하고, '-'로 시작해야 합니다. 문장 중간에 '-'를 사용하여 내용을 구분하지 마세요.** 모든 문장은 **명사형으로 간결하게 종결**해야 합니다(예: '불안감 조성', '특정 집단에 대한 부정적 이미지 형성함'). 핵심 내용은 '**'로 감싸 굵게 표시하고, 특히 강조하고 싶은 부분은 '__'로 감싸 밑줄을 쳐서 표시해주세요.
  ...
  properties: {
    ...
    comprehensiveAnalysis: {
        ...
        properties: {
            ...
            criticalQuestions: {
                type: Type.STRING,
                description: "독자가 콘텐츠를 비판적으로 수용하기 위해 생각해볼 질문과 제안. '해독제' 역할을 하며, 명시된 특별한 형식(핵심질문 + 3가지 생각거리)을 따릅니다."
            }
        },
